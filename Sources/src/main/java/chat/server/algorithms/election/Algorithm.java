...
public enum Algorithm implements Action<State> {
  ...
  LEADER_MESSAGE(ElectionLeaderContent.class) {
     ...
     */
    public void execute(final State state, final AbstractContent content) {
      Actions.receiveLeaderContent(state, (ElectionLeaderContent) content);
    }
  };
   ...
   * are actions of the algorithms (subtypes of {@link chat.common.Action}); it
   * is thus {@code static}. The collection is unmodifiable and the attribute is
