...
public final class Log {
	...
	public static void configureALogger(final String loggerName,
			final Level level) {
		String name = Optional.ofNullable(loggerName)
				.orElseThrow(IllegalArgumentException::new);
		if (name.equalsIgnoreCase(LOGGER_NAME_GEN)) {
			GEN.setLevel(level);
		} else if (name.equalsIgnoreCase(LOGGER_NAME_COMM)) {
			COMM.setLevel(level);
		} else if (name.equalsIgnoreCase(LOGGER_NAME_TEST)) {
			TEST.setLevel(level);
		} else if (name.equalsIgnoreCase(LOGGER_NAME_CHAT)) {
			CHAT.setLevel(level);
		} else if (name.equalsIgnoreCase(LOGGER_NAME_ELECTION)) {
