...
public abstract class Scenario {
	...
	/**
	 * instanciates a client. To match the arguments of the constructor of a
	 * client, the server host name is {@code localhost}.
	 * 
	 * @param serverPortNb
	 ...
	 *            the line inputed.
	 * @throws IOException
	 *             the exception thrown when a communication problem occurs
	...
	public void emulateAnInputLineFromTheConsoleForAClient(final Client client,
		...
		Client c = Optional.ofNullable(client)
				.orElseThrow(IllegalArgumentException::new);
		String line = Optional.ofNullable(inputLine)
				.orElseThrow(IllegalArgumentException::new);
		c.treatConsoleInput(line);
	}

	/**
	 * instanciates a server. For the sake of simplicity, the arguments are
	 * similar to the ones of the constructor: the string value
	 * {@code localhost} must be provided for every neighbouring server to which
	 * the new server connects to.
	 * 
	 * @param args
	 *            the argument of the server as written when starting the server
	...
	public Server instanciateAServer(final String args) {
		Server chatServer = new Server(Optional.ofNullable(args)
				.map(a -> a.split("\\s+")).orElse(new String[0]));
	...
	public void emulateAnInputLineFromTheConsoleForAServer(final Server server,
		...
		Server s = Optional.ofNullable(server)
				.orElseThrow(IllegalArgumentException::new);
		String line = Optional.ofNullable(inputLine)
				.orElseThrow(IllegalArgumentException::new);
		s.treatConsoleInput(line);
	}

	/**
	 * constructs and run the scenario.
	 * 
	 * NB: for now, we do not separate the sequence of actions that create
	 * clients and servers from the sequence of actions that exercise the
	 * algorithms.
	 * 
	 * @throws Exception
	...
	/**
	 * is a utility method to encapsulate {@code Thread.sleep} with an log
	 * message in case of an {@code InterruptedException} is thrown.
	 * 
	 * @param millis
