...
public class VectorClock implements Serializable, Cloneable {
	 ...
	 */
	public int setEntry(final Integer key, final int value) {
		if (key < 0 || value < 0) {
			throw new IllegalArgumentException(
					"arguments must be greater or equal to 0 (" + key + ", "
							+ value + ")");
		}
	...
	public void max(final VectorClock other) {
		Consumer<Integer> m = (i) -> {
			setEntry(i,
					Math.max(vectorClock.getOrDefault(i, 0),
							Optional.ofNullable(other)
									.map(o -> o.vectorClock.getOrDefault(i, 0))
									.orElse(0)));
		};
		Stream.concat(vectorClock.keySet().stream(),
				Optional.ofNullable(other)
						.map(o -> o.vectorClock.keySet().stream())
						.orElse(Stream.empty()))
				.distinct().forEach(m);
		assert invariant();
	}
