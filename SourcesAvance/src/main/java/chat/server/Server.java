...
public class Server {
	 ...
	 */
	public Server(final String[] argsFromMain) {
		String[] args = Optional.ofNullable(argsFromMain)
	...
	public void addServer(final String host, final int port)
			throws IOException {
		String h = Optional.ofNullable(host)
				.orElseThrow(IllegalArgumentException::new);
		if (LOG_ON && COMM.isInfoEnabled()) {
			COMM.info("Opening connection with server on host " + h
					+ " on port " + port);
		}
		InetAddress destAddr = InetAddress.getByName(h);
		SocketChannel rwChan = SocketChannel.open();
		Socket rwSock = rwChan.socket();
		// obtain the IP address of the target host
		InetSocketAddress rcvAddress = new InetSocketAddress(destAddr, port);
	...
	public void acceptNewServer(final ServerSocketChannel sc)
			throws IOException {
		SocketChannel rwChan = Optional.ofNullable(sc).map(s -> {
			try {
				return s.accept();
			} catch (IOException e) {
				COMM.error(e.getLocalizedMessage());
	...
	private void forwardServers(final SelectionKey exceptKey, final int type,
		...
		final Consumer<SelectionKey> send = key -> {
			...
			try {
				...
				if (LOG_ON && COMM.isInfoEnabled()) {
					COMM.info("Send message to a server end point");
				}
			} catch (IOException e) {
	...
	private void forwardClients(final SelectionKey exceptKey, final int type,
		...
		final Consumer<SelectionKey> send = (key) -> {
			...
			try {
				worker.sendMsg(type, identity, seqNumber, msg);
				if (LOG_ON && COMM.isInfoEnabled()) {
					COMM.info("Send message to a client end point");
				}
